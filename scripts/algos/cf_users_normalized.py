...
reader = csv.DictReader(ftr, delimiter='\t')
for row in reader:
  Utility[int(row['uid'])-1][int(row['mid'])-1] = int(row['ratings']) - Average[int(row['uid'])-1]
#######
...
for row in reader:
  ...
  try:
    ...
    rating = Average[int(row['uid'])-1] + predictRating(int(row['uid'])-1, int(row['mid'])-1)
...
print ("Root Mean Squared Error is %f") % (mean_absolute_error(y_true, y_pred))
